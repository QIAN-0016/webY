	...
	$.fn.slotMachine = function(settings){
		...
		*/
		function _stop( nowOrRepeations, getElementFn ){
			...
			if( typeof getElementFn==="function" ){
				rnd = getElementFn();
			}else if(typeof getElementFn==="object"){
				rnd = getElementFn;
			}else{
				if( settings.repeat ){
					rnd = _getNext();
		...
		*/
		...
		/**
		  ...
		  * @param int count - Number of shuffles (undefined to make infinite animation
		*/
		$slot.shuffle = function( count,pos,oncomplete ){
			_forceStop = false;
		...
		*/
		...
		/**
		 * @desc PUBLIC - 指定停止在某个地方
		 * 使用方法与插件原有的 prev和next方法一样。
		*/
		/*$slot.appolit = function(){
		    _stop(true,_appolit);
		};*/
		function  _appolit(n){
		    var appIndex = n<$titles.length ? n : 0;
